...
int move_piece(ChessBoard* board, int y0, int x0, int y1, int x1) {
	...
	if(is_stalemate(board, board->whites_turn)) {
		flag = 4; // Code for stalemate
	} else if (is_checkmate(board, board->whites_turn)){
		flag = 5 + (int)board->whites_turn; // Code for checkmate, 6 if black wins, 5 if white wins
	}
...
void get_moves_pawn(int* moves, ChessBoard *board, int y, int x, bool white) {
	...
	if (white) {
		...
		// en passant
		if (board->lastPawnDoubleMove && y == 4 && abs((board->lastMovePawnIndex % 8) - x) == 1 
		&& board->lastMovePawnIndex / 8 == 6) {
		...
		// en passant
		if (board->lastPawnDoubleMove && y == 3 && abs((board->lastMovePawnIndex % 8) - x) == 1 
		&& board->lastMovePawnIndex / 8 == 1) {
...
void check_legal_moves_king(int* moves, ChessBoard* board, int y0, int x0, bool white) {
	...
	for(int i = 1; i <= moves[0]; ++i){
		...
		if (y0 == king_y && x0 == king_x && !is_under_attack(board, y1, x1, white))
